...
module towers() {
    for (i = [0 : 24]) 
        ...
        randomWallTower(randomizedIboundPos(seed, i-1)*sf, randomizedIboundPos(seed, i)*sf, randomizedIboundPos(seed, i+1)*sf, i*4+2);

    for (i = [26 : 35]) 
        translate(randomizedOboundPos(seed, i)*sf)
        randomWallTower(randomizedIboundPos(seed, i-1)*sf, randomizedIboundPos(seed, i)*sf, randomizedIboundPos(seed, i+1)*sf, i*5+838);
}
